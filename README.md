...
<body>
  <div class="quiz-container">
    <h1>Kuis Pengetahuan Arduino Dasar</h1>
    <button id="start-btn" class="btn">Mulai</button>
    <div id="question-container" class="hide">
      <div id="question" style="margin: 20px 0;"></div>
      <div id="answer-buttons" class="btn-grid"></div>
    </div>
    <div id="completion-message" class="hide">Selamat! Kuis selesai 🎉</div>
  </div>
  ...
  { "en": "Ikonoklasme?", "id": "Penghancuran berhala." }
    ...
    function showQuestion() {
      ...
      questionElement.innerText = question.en;

      const correctAnswer = question.id;
      const otherChoices = questions
        .filter(q => q.id !== correctAnswer)
        .sort(() => Math.random() - 0.5)
        .slice(0, 3)
        .map(q => q.id);

      const choices = [correctAnswer, ...otherChoices].sort(() => Math.random() - 0.5);
    ...
    function selectAnswer(button, isCorrect) {
      ...
      allButtons.forEach(btn => {
        btn.disabled = true;
        if (btn.innerText === shuffledQuestions[currentQuestionIndex].id) {
      ...
      setTimeout(() => {
        ...
        if (currentQuestionIndex < shuffledQuestions.length) {
          ...
          startButton.innerText = 'Mulai Lagi';
